...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if method == 'merge':  # Merge NMS (boxes merged using weighted mean)
            ...
            for _ in i:
                if int(x[i[id_i], -1]) == 0:
                    point.append(x[i[id_i]])
                id_i += 1
            id_i = 0
            for _ in i:
                if int(x[i[id_i], -1]) == 1:
                    line.append(x[i[id_i]])
                id_i += 1
            id_i = 0
            for _ in i:
                if int(x[i[id_i], -1]) == 2:
                    area.append(x[i[id_i]])
                    # print(area)
                id_i += 1
            # print(point)
            if area:
                ...
                for a in area:
                    ...
                    y1_1.append(a[3])
                    con.append(a[4])
                x1_new = min(x1).cpu().item()
                y1_new = min(y1).cpu().item()
                x1_1_new = max(x1_1).cpu().item()
                y1_1_new = max(y1_1).cpu().item()
                con_new = max(con).cpu().item()
                y.append(x1_new)
                y.append(y1_new)
                y.append(x1_1_new)
                y.append(y1_1_new)
                y.append(con_new)
                y.append(area[0][-1].cpu().item())
                yy.append(y)
                # yy = torch.Tensor(yy).to('cuda:0')
                # print(x[i])
                # print(yy)
            if line:
                x1 = []
